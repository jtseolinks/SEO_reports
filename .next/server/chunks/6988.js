...
</html>`}},65430:(a,b,c)=>{c.d(b,{I:()=>j,M:()=>i});var d=c(33873),e=c.n(d),f=c(57367),g=c.n(f),h=c(15604);async function i(a,b,d){let f=(0,h.GO)(b);await g().mkdir(f,{recursive:!0});let i=e().join(f,e().basename(d)),j=await Promise.resolve().then(c.bind(c,83636)),k=await j.default.launch({headless:!0,args:["--no-sandbox","--disable-setuid-sandbox","--disable-dev-shm-usage"]});try{let b=await k.newPage();await b.setContent(a,{waitUntil:"load"}),await b.pdf({path:i,format:"A4",printBackground:!0,margin:{top:"0",right:"0",bottom:"0",left:"0"}})}finally{await k.close()}return(0,h.hv)(b,d)}function j(a,b){let c=b.replace(/-/g,"_");return`report_${a}_${c}.pdf`}}};